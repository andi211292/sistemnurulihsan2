...
export default function SantriPage() {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const res = await apiFetch("http://50.50.50.10:8080/api/students/import_excel", {
                method: "POST",
                body: formData
            });
    ...
    const handleCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const res = await apiFetch("http://50.50.50.10:8080/api/students/import_csv", {
                method: "POST",
                body: formData
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (modalMode === "add") {
                ...
                const res = await apiFetch(`http://50.50.50.10:8080/api/students/${formData.id}`, {
                    method: "PUT",
                    body: JSON.stringify(payload)
                });
    ...
    const handleDelete = async (student_id: number, name: string) => {
        ...
        try {
            setLoading(true);
            const res = await apiFetch(`http://50.50.50.10:8080/api/students/${student_id}`, {
                method: "DELETE"
            });
            if (!res.ok) {
                const errData = await res.json();
